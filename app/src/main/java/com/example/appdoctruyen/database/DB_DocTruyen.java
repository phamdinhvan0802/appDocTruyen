...
public class DB_DocTruyen extends SQLiteOpenHelper {
    ...
    // tạo bảng
    public DB_DocTruyen(@Nullable Context context) {
    ...
    // lấy 3 truyện
    public List<Truyen> getData1(){
        List<Truyen> list = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();

        String sqlSelect = " SELECT * FROM truyen ORDER BY idtruyen DESC LIMIT 3 ";
        ...
        while (cursor.moveToNext()){
            ...
            list.add(new Truyen(id, tentruyen, noidung, anh, id_tk));
        }
        cursor.close();
    ...
    // lấy tất cả truyện
    public List<Truyen> getData2(){
        List<Truyen> list = new ArrayList<>();

        SQLiteDatabase db = this.getReadableDatabase();

        String sqlSelect = " SELECT * FROM truyen";
